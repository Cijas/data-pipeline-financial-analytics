    ...
    def run(self, source: str = "mock", n_transactions: int = 100000) -> dict:
        ...
        try:
            ...
            quality_metrics = {
                ...
                "data_loss_percent": ((len(raw_df) - len(transactions)) / len(raw_df) * 100),
                "unique_customers": len(customers),
                "unique_categories": len(categories),
                "date_range": f"{transactions['transaction_date'].min().date()} to {transactions['transaction_date'].max().date()}",
